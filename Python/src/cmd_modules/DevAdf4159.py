...
class DevAdf4159(DevDriver.DevDriver):    
    ...
    def     DevSetReg(self, Regs):
        ...
        if self.USpiCfg_Type == 'USpi':
            ...
            Brd = self.Brd();
            if (Brd is not None):
                Ret     =   Brd.Fpga_SetUSpiData(dUSpiCfg, Regs)
        return Ret
    ...
    def     GenRegs(self):
        ...
        BandwidthDiv                =   fStopDiv - fStrtDiv                                                 # Bandwidth divided
        ...
        NrSteps                     =   ceil(self.TRampUp/Tim)
        DevMax                      =   2**15
        fRes                        =   fPfd/(2**25)

        fDevVal                     =   round(BandwidthDiv/NrSteps)
        DevVal                      =   round(fDevVal/fPfd*(2**25))
        DevOffs                     =   -1
        ...
        BUp                         =   2*fPfd/(2**25)*(DevArg*(2**DevOffs))*NrSteps

        self.fStrt                   =   fStrt
        self.fStop                   =   fStrt + BUp
        self.TRampUp                 =   Tim*NrSteps  
        self.TRampDo                 =   Tim*NrSteps 

        print('TRampUp: ', self.TRampUp/1e-6)
        print('fStrt: ', self.fStrt/1e6)
        print('fStop: ', self.fStop/1e6)            
        

        Data                =   zeros(11)
        Data[0]             =   self.GenRegFlag('R7_Delay', 0, 'TxTrig',1)
        Data[1]             =   self.GenRegFlag('R6_Step', 0, 'StepWrd', NrSteps, 'StepSel', 0)
        Data[2]             =   self.GenRegFlag('R6_Step', 0, 'StepWrd', NrSteps, 'StepSel', 1)
        Data[3]             =   self.GenRegFlag('R5_Dev', 0, 'DevWrd', DevVal, 'DevOffs', DevOffs, 'DevSel', 0)
        Data[4]             =   self.GenRegFlag('R5_Dev', 0, 'DevWrd', DevVal, 'DevOffs', DevOffs, 'DevSel', 1)
        Data[5]             =   self.GenRegFlag('R4_Clk', 0, 'ClkDivSel', 0, 'Clk2Div', Clk2, 'ClkDivMod', 3)
        Data[6]             =   self.GenRegFlag('R4_Clk', 0, 'ClkDivSel', 1, 'Clk2Div', Clk2, 'ClkDivMod', 3)
        Data[7]             =   self.GenRegFlag('R3_Func', 0, 'PDPol', 1, 'RampMod', 2)
        Data[8]             =   self.GenRegFlag('R2_Div', 0, 'Clk1Div', Clk1, 'RCntr', self.ValR, 'RefDoub', self.ValD, 'RDiv2', self.ValT, 'PreSca', 1, 'CP', 11)

...
import  src.cmd_modules.Rbk2Fft as Rbk2Fft 
...
class Rbk2Adf24Tx2Rx8Fft(Rbk2Fft.Rbk2Fft):

    def __init__(self, stConType, *args):
        super(Rbk2Adf24Tx2Rx8Fft, self).__init__(stConType, *args)
        ...
        self.Rad_ArmMemCfg_WaitLev = 32768/2 - 4096;
        self.Rad_ArmMemCfg_FifoSiz = 32768/2;
            ...
            if stMod == 'ExtTrigUp':
                ...
                if self.Gps > 0:
                    if self.Rad_EnaFrmCntr > 0:
                        self.Rad_FrmCtrlCfg_RegChnCtrl = self.cFRMCTRL_REG_CH0CTRL_FRMCNTRENA + self.cFRMCTRL_REG_CH0CTRL_WAITENA + self.cFRMCTRL_REG_CH0CTRL_PADENA
                    else:
                        self.Rad_FrmCtrlCfg_RegChnCtrl = self.cFRMCTRL_REG_CH0CTRL_WAITENA + self.cFRMCTRL_REG_CH0CTRL_PADENA
                else:
                    if self.Rad_EnaFrmCntr > 0:
                        self.Rad_FrmCtrlCfg_RegChnCtrl = self.cFRMCTRL_REG_CH0CTRL_FRMCNTRENA + self.cFRMCTRL_REG_CH0CTRL_WAITENA     
                    else:
                        self.Rad_FrmCtrlCfg_RegChnCtrl = self.cFRMCTRL_REG_CH0CTRL_WAITENA;
                self.BrdSampIni()
    ...
    def     BrdSetTim_MxPW(self, dCfg):
        ...
        if dCfg['MeasNp'] <= 256:
            ...
            if dCfg["IniEve"] > 0:
                ...
                lSeq = Seq.IniSeq('IniNoFrm', dCfg["IniTim"]) 

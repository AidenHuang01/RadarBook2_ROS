...
# Version 1.6.3
#     ConSetFileParam with long arrays
#     HdfSplit
...
# Version 1.7.0
#     Compared with Matlab 3.1.6
#     Added ConSet('InputType') to support Fft and LoMax
...
import  src.cmd_modules.Computation as Computation
...
class Connection():
    ...
    def __init__(self, devType, stConType='Usb', *args):
        ...
        self.cReplayExt         =   -1
        self.cExtSize           =   0
        self.cExtMult           =   0           # public in matlab
        self.cDataIdx           =   0
        ...
        if self.cType == 'Usb':
            ...
            if self.hCon > -1:
                ...
                if (Err > 0) and (Len > 0): 
                    ...
                    if (stopOnError):
                        raise ConnectionError(Ret.decode('ASCII'));
                    elif (dispError):
    ...
    def     ConSetFileParam(self, stName, Val, DataType='STRING'):
        if  self.cType == 'RadServe':
            ...
            LenMeta = int(2 + len(stName)/4);
            if (DataType.upper() == 'INT' or isinstance(Val, int)):
                Data = zeros(LenMeta + 1);
                Data[0] = 1; # type
                Data[1] = uint32(len(stName));
                ...
                if (LenMeta + 1 + len(Val) > 254):
                    # cmd too long -> split into multiple packets
                    Len = 254 - LenMeta - 3;
                    NumPackets = int(ceil(len(Val) / Len));
                    DataStrt = 0;
                    DataStop = Len;
                    Append   = 0;
                    for IdxPack in range(0, NumPackets):
                        Data = zeros(LenMeta + 3 + Len);
                        Data[0] = uint32(3); # type
                        Data[1] = uint32(len(stName));
                        for Idx in range(0,len(Name)):
                            Data[2 + Idx] = Name[Idx];
                        Data[LenMeta] = uint32(0);
                        Data[int(LenMeta + 1)] = uint32(Append);
                        Data[int(LenMeta + 2)] = uint32(Len);
                        for Idx in range(0, Len):
                            Data[int(LenMeta + 3 + Idx)] = uint32(Val[DataStrt + Idx]);
                                                   
                        Ret = self.CmdSend(0, int('0x6105',0), Data, 0);
                        Ret = self.CmdRecv();
                        ...
                        if (DataStop > len(Val)):
                            DataStop = len(Val) - 1;
                            Len = DataStop - DataStrt + 1;
                            
                    return;
                else:
                    # no split necessary
                    Data = zeros(LenMeta + 1 + len(Val));
                    Data[0] = 3; # type
                    Data[1] = uint32(len(stName));
                    for Idx in range(0,len(Name)):
                        Data[2 + Idx] = Name[Idx];
                ...
                if ((LenMeta + 1 + len(Val)*2) > 254):
                    # cmd too long -> split into multiple packets
                    Len = 254 - LenMeta - 3;
                    if (Len % 2 != 0):
                        Len = Len - 1;
                    Len = int(Len / 2); # each array element takes two fields
                    NumPackets = int(ceil(len(Val) / Len));
                    DataStrt = 0;
                    DataStop = Len;
                    Append   = 0;
                    for IdxPack in range(0, NumPackets):
                        Data = zeros(LenMeta + 3 + Len * 2);
                        Data[0] = uint32(4); # type
                        Data[1] = uint32(len(stName));
                        for Idx in range(0,len(Name)):
                            Data[2 + Idx] = Name[Idx];
                        Data[LenMeta] = uint32(0);
                        Data[int(LenMeta + 1)] = uint32(Append);
                        Data[int(LenMeta + 2)] = uint32(Len);
                        valArray = Val[DataStrt:DataStop].tostring();
                        Data[int(LenMeta + 3):int(LenMeta + 3 + Len * 2)] = fromstring(numpy.ascontiguousarray(Val[DataStrt:DataStop]), dtype=numpy.uint32);
                        ...
                        if (DataStop > len(Val)):
                            DataStop = len(Val);
                            Len = DataStop - DataStrt;
                            
                    return;
                else:
                    Data = zeros(LenMeta + 1 + len(Val)*2);
                    Data[0] = 4; # type
                    Data[1] = uint32(len(stName));
                    for Idx in range(0,len(Name)):
                        Data[2 + Idx] = Name[Idx];
                ...
                if (LenMeta + len(Value) > 254):
                    # split into packets
                    NumPackets = int(ceil(len(Value) / (254 - (LenMeta  + 3))));
                    Len = 254 - LenMeta - 3;
                    DataStrt = 0;
                    DataStop = Len;
                    Append = 0;
                    for IdxPack in range(0, NumPackets):
                        ...
                        for Idx in range(0,len(Name)):
                            Data[2 + Idx] = Name[Idx];
                        Data[LenMeta] = uint32(0); # split
                        Data[int(LenMeta + 1)] = uint32(Append);
                        Data[int(LenMeta + 2)] = uint32(Len);
                        for Idx in range(0,Len):
                            Data[int(LenMeta + 3 + Idx)] = Value[int(DataStrt + Idx)];
    ...
    def     ConGetData(self, Len, DType='int16'):
        if self.hConDat > -1:
            ...
            try:
                ...
                if (DType == 'int16'):
                    ...
                    Ret = ndarray((int(Len/4)), buffer=ba, dtype='int32')
    ...
    def     ConSet(self,*varargin):
        if len(varargin) > 0:
            ...
            if stVal == 'BufSiz':
                ...
                if len(varargin) > 1:
                    if self.cType == 'RadServe':
                        Ret         =   self.CmdSend(0, int('0x6111',0),[uint32(0), uint32(varargin[1])],0);
                        Ret         =   self.CmdRecv();
            elif stVal == 'HdfSplitNumZeros':
                if len(varargin) > 1:
                    if self.cType == 'RadServe':
                        Ret         =   self.CmdSend(0, int('0x6111',0),[uint32(1), uint32(varargin[1])],0);
                        Ret         =   self.CmdRecv();
            elif stVal == 'InputType':
                if len(varargin) > 1:
                    if self.cType == 'RadServe':
                        stType = varargin[1];
                        Type = 0;
                        ...
                        Ret         =   self.CmdSend(0, int('0x611D',0),[uint32(Type)], 0);
                        Ret         =   self.CmdRecv();
                        
                        if len(varargin) > 2:
                            NTx = varargin[2];
                            self.ConSetFileParam('N', uint32(NTx), 'INT');
                            if len(varargin) > 3:
                                Mult = varargin[3];
                                self.ConSet('Mult', uint32(Mult));
                                self.Computation.SetParam('DL_NummmDetections', Mult);
    ...
    def     DispVideo(self, VidIdx=0):
        if (0 <= VidIdx and VidIdx < len(self.hConVideo)):
            if (self.hConVideo[VidIdx] > 0 and self.cVideoRate[VidIdx] != 0):
                if ((self.cVideoFrmsColl[VidIdx] == 0) and (self.cDataIdx > self.cVideoRate[VidIdx])) or ((self.cVideoFrmsColl[VidIdx] > 0) and (self.cDataIdx > (self.cVideoRate[VidIdx] * (self.cVideoFrmsColl[VidIdx] + 2)))):
        ...
        for VidIdx in range(0, len(self.hConVideo)):
            ...
            if (self.hConVideo[VidIdx] > 0 and self.cVideoRate[VidIdx] != 0):
                if self.cExtSize == 0:
                    ...
                    if ( ( ((self.cDataIdx + 1) * self.cExtMult) - self.cVideoFrmsColl[VidIdx] * self.cMult ) / (self.cVideoRate[VidIdx]) >= 1):
                        readVid = True;

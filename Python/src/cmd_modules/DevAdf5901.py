...
class DevAdf5901(DevDriver.DevDriver):
    ...
    def     DevSetReg(self, Regs):
        ...
        if self.USpiCfg_Type == 'USpi':
            ...
            Brd = self.Brd();
            if (Brd is not None):
                Brd.Fpga_SetUSpiData(dUSpiCfg, Regs[0:13])
                Brd.Fpga_SetUSpiData(dUSpiCfg, Regs[13:15])
                Brd.Fpga_SetUSpiData(dUSpiCfg, Regs[15:17])
                Ret                 =   Brd.Fpga_SetUSpiData(dUSpiCfg, Regs[17:19])
    ...
    def     GenRegs(self):   
        ...
        #--------------------------------------------------------------
        Data[3]         =   self.GenRegFlag('R8', 0, 'FreqCalDiv', 1000)
        #--------------------------------------------------------------
        # Initialize Register 0:
        # Reserved
        #--------------------------------------------------------------
        Data[4]         =   self.GenRegFlag('R0', 0, 'AuxBufGain', 4, 'AuxDiv', 1, 'PupVco', 1, 'PupAdc', 1)
        #--------------------------------------------------------------
        # Initialize Register 7:
        # Mater reset
        #--------------------------------------------------------------
        Data[5]         =   self.GenRegFlag('R7', 0, 'RDiv', self.RfRefDiv, 'ClkDiv', 1000)


        RefClk          =   self.RfSysClk/self.RfRefDiv
        ...
        # Reserved: Frac MSB:
        #--------------------------------------------------------------
        Data[7]         =   self.GenRegFlag('R5', 0, 'FracMsb', DivFracMsb, 'Int', DivInt)
        #--------------------------------------------------------------
        # Initialize Register 4:
        # Analog Test Bus Configuration
        #--------------------------------------------------------------
        Data[8]         =   self.GenRegFlag('R4', 0)
        #--------------------------------------------------------------
        ...
        # Adc configuration
        #--------------------------------------------------------------
        Data[10]        =   self.GenRegFlag('R2', 0, 'AdcClkDiv', 80, 'AdcAv', 0)
        #--------------------------------------------------------------
